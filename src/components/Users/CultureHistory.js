...
const CultureHistory = () => {
  ...
  const [reviews, setReviews] = useState([
    {
      ...
      rating: 5,
      comment: "The cultural tour was amazing! Learned so much about the local history and traditions. Highly recommend!",
    },
    {
      reviewer: "David Black",
      rating: 4,
      comment: "Very informative and enjoyable. The guide was knowledgeable and friendly. A great experience overall.",
    },
  ]);
  ...
  useEffect(() => {
    const fetchActivities = async () => {
      try {
        const placesCollectionRef = collection(db, "categories", "Culture & History", "places");
  ...
  return (
    <section className="py-16 bg-gradient-to-r from-yellow-100 via-red-100 to-orange-100">
      <h2 className="text-4xl font-bold mb-12 text-center text-red-800">Culture & History</h2>

      {activities.length === 0 ? (
        <p className="text-center text-lg text-gray-700">No cultural or historical activities available at the moment.</p>
      ) : (
          ...
          {activities.map((activity, index) => (
            <div key={index} className="group rounded-lg shadow-xl overflow-hidden bg-white transform transition-transform duration-500 hover:scale-105">
              ...
              <div className="p-6">
                ...
                <p className="text-gray-700 mb-4">
                  <span className="font-bold text-red-800">Entrance Fee:</span> {activity.entranceFee || "No entrance fee information available."}
                </p>
                <p className="text-gray-700 mb-4">
