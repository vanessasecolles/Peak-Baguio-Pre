...
const DynamicSpotAdmin = () => {
  ...
  return (
    <div className="p-4 bg-gray-100 min-h-screen">
      ...
      >
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-bold">{currentItem ? "Edit Item" : "Add New Item"}</h3>
          <button onClick={() => setIsModalOpen(false)}>
            <FontAwesomeIcon icon={faTimes} size="lg" />
          </button>
        </div>
        <input className="w-full mb-3 p-2 border rounded" name="name" placeholder="Name" value={currentItem?.name || newItem.name} onChange={handleInputChange} />
        <textarea className="w-full mb-3 p-2 border rounded" name="description" placeholder="Description" value={currentItem?.description || newItem.description} onChange={handleInputChange}/>
        <input className="w-full mb-3 p-2 border rounded" name="image" placeholder="Image URL" value={currentItem?.image || newItem.image} onChange={handleInputChange} />
        <input className="w-full mb-3 p-2 border rounded" type="number" name="price" placeholder="Price" value={currentItem?.price || newItem.price} onChange={handleInputChange}/>
        <button className="bg-teal-500 text-white rounded px-4 py-2 float-right" onClick={handleAddOrUpdate}>
