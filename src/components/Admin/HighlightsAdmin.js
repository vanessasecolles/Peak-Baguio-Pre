...
const HighlightsAdmin = () => {
  ...
  useEffect(() => {
    const unsubscribeHighlights = onSnapshot(highlightsCollection, (snapshot) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        const docRef = doc(db, "highlights", editId);
        await updateDoc(docRef, formData);
        alert("Highlight updated successfully!");
      } else {
        await addDoc(highlightsCollection, formData);
      ...
      alert("There was an error while saving the highlight. Please try again.");
    }
  };
  ...
  const handleEdit = (highlight) => {
    setFormData({
      ...
      isFeatured: highlight.isFeatured || false,
  ...
  const handleDelete = async () => {
    if (deleteId) {
      try {
        const docRef = doc(db, "highlights", deleteId);
        await deleteDoc(docRef);
        alert("Highlight deleted successfully!");
        closeDeleteModal();
      } catch (error) {
        console.error("Error deleting highlight:", error);
        alert("There was an error while deleting the highlight. Please try again.");
      }
    }
  };
  ...
  return (
    <div className="p-8 bg-gradient-to-r from-teal-100 via-blue-100 to-teal-50 min-h-screen">
      <div className="bg-white shadow-xl rounded-lg p-8 mb-10 transform hover:scale-105 transition-transform duration-500 ease-in-out">
        <h2 className="text-4xl font-bold mb-6 text-center text-teal-700">Manage Highlights</h2>
        <button
          onClick={() => {
            resetForm();
            setIsModalOpen(true);
      ...
      <div className="bg-white shadow-xl rounded-lg p-8">
        <h3 className="text-3xl font-bold mb-6 text-center text-teal-700">Existing Highlights</h3>
        {highlights.length === 0 ? (
          <p className="text-center text-teal-800">No highlights found. Please add some highlights.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {highlights.map((highlight) => (
              <div key={highlight.id} className="border rounded-lg p-6 shadow-lg bg-white transform hover:scale-105 transition-transform duration-500 ease-in-out">
                {highlight.image && (
                  <img
                    src={highlight.image}
                    alt={highlight.title}
                ...
                <h4 className="text-2xl font-semibold mb-4 text-teal-800">{highlight.title} {highlight.isFeatured && <span className='text-sm text-yellow-500'>(Featured)</span>}</h4>
                <p className="text-gray-700 mb-4">{highlight.description}</p>
                <p className="text-gray-600 mb-4">Category: {highlight.category}</p>
                <div className="flex justify-between">
                  ...
                  <button
                    onClick={() => openDeleteModal(highlight.id)}
      ...
      <Modal
        ...
        contentLabel="Add/Edit Highlight Modal"
      ...
      >
        ...
        <h3 className="text-3xl font-bold mb-6 text-teal-700 text-center">{editId ? "Edit Highlight" : "Add New Highlight"}</h3>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="flex flex-col">
            <label className="font-semibold mb-2 text-teal-800">Highlight Title</label>
            <input
              type="text"
              placeholder="Title"
          ...
          <div className="flex items-center space-x-4">
            ...
            />
            <label className="font-semibold text-teal-800">Featured Highlight</label>
          </div>
          <button
            type="submit"
            className="bg-teal-700 text-white py-4 px-6 rounded-lg hover:bg-teal-800 focus:outline-none focus:ring-4 focus:ring-teal-500 flex items-center justify-center space-x-2 transition-transform transform hover:scale-105 duration-300 ease-in-out"
          >
            <FontAwesomeIcon icon={faPlus} />
            <span>{editId ? "Update Highlight" : "Add Highlight"}</span>
          </button>
        </form>
      </Modal>
      ...
      <Modal
        ...
        contentLabel="Delete Highlight Confirmation"
        className="modal-content bg-white p-8 rounded-lg shadow-2xl max-w-md mx-auto transform hover:scale-105 transition-transform duration-500 ease-in-out"
        overlayClassName="modal-overlay fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center bg-black bg-opacity-50"
      >
        <h3 className="text-3xl font-bold mb-6 text-red-700 text-center">Delete Highlight</h3>
        <p className="text-gray-700 text-lg text-center mb-8">Are you sure you want to delete this highlight?</p>
